...
const mulai = async (sesi, conn = new WAConnection()) => {
     ...
     const responseWeb = await Axios.get('https://web.whatsapp.com/check-update?version=0&platform=web')
     const resData = responseWeb.data
     conn.version = resData.currentVersion.split('.').map(i => Number(i))
